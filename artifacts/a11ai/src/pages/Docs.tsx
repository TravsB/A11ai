      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[220px_1fr]">
        <aside className="lg:sticky lg:top-24 lg:self-start">
          ...
          <nav className="mt-3 space-y-1">
            {NAV.map((n) => (
              <a key={n.id} href={`#${n.id}`} className="block rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted hover:text-ink">
                {n.label}
              </a>
            ))}
          </nav>
        ...
        <div className="space-y-10">
          <section id="install">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Install via npm or load the universal script.</p>
            <CodeBlock code={`npm install @a11ai/engine`} lang="bash" />
            <CodeBlock code={`<script src="https://cdn.a11ai.io/v3/engine.js" data-key="va_pk_live_..."></script>`} lang="html" />
          </section>
...
const engine = A11ai.init({
  apiKey: process.env.A11AI_PUBLIC_KEY!,
...
engine.on("issue", (e) => console.warn(e.severity, e.message));`}
          ...
          <section id="extension">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              The Chrome (Manifest V3) extension applies the same engine to any website your users visit.
            </p>
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-lg border border-border bg-ink text-background">
      <div className="flex items-center justify-between border-b border-background/10 px-4 py-2">
        ...
        <button onClick={copy} className="flex items-center gap-1.5 text-xs text-background/50 hover:text-background">
