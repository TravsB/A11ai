...
function buildPalette(brand: string) {
  ...
  return labels.map((label, i) => {
    ...
    const hex = blendHex("#FFFFFF", brand, t);
    const lum = hexToLum(hex);
    const fg = lum > 0.35 ? "#0D0D0D" : "#FFFFFF";
    return { label, hex, fg };
  });
}
...
function Pill({ ok }: { ok: boolean }) {
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${ok ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
      {ok ? "Pass" : "Fail"}
    </span>
  );
}
...
export default function Palette() {
  ...
  const tokens = palette.map((c) => `  --color-${c.label}: ${c.hex};`).join("\n");
  const css = `:root {\n${tokens}\n}`;
    ...
    <div className="mx-auto max-w-7xl px-6 py-10">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[360px_1fr]">
        <aside className="rounded-2xl border border-border bg-surface-elevated p-5 shadow-soft">
          <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Brand seed</label>
          ...
          <div className="mt-5">
            <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Presets</div>
          ...
          <div className="mt-5">
            <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Preview through</div>
            <div className="mt-2 grid grid-cols-2 gap-1">
              {VISION_MODES.map((m) => (
                <button
                  key={m.id}
                  onClick={() => setMode(m.id)}
                  className={`rounded-md px-2 py-1.5 text-xs font-medium ${mode === m.id ? "bg-ink text-background" : "bg-surface text-muted-foreground hover:text-ink"}`}
                >
                  {m.label}
                </button>
              ))}
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-surface-elevated p-6 shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">WCAG 2.2 AA · {palette.length} swatches</span>
            </div>
            <div className={`mt-5 grid overflow-hidden rounded-xl border border-border ${visionClass(mode)}`} style={{ gridTemplateColumns: `repeat(${palette.length}, 1fr)` }}>
              {palette.map((c, i) => (
          ...
          <div className="rounded-2xl border border-border bg-ink p-6 text-background shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <button onClick={copy} className="flex items-center gap-1.5 text-xs text-background/60 hover:text-background">
                <Copy className="h-3.5 w-3.5" />
                {copied ? "Copied!" : "Copy CSS"}
              </button>
            </div>
            <pre className="mt-4 overflow-x-auto text-xs leading-relaxed text-background/70">
              <code>{css}</code>
            </pre>
          </div>
        </div>
