...
interface QuickScanResult {
  score: number;
  totalIssues: number;
  preview: Array<{ id: string; severity: string; title: string; count: number }>;
  teaser: string | null;
}
...
function QuickScanWidget() {
  ...
  async function handleScan(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/public/scan-preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: url.trim() }),
      });
      const data = await res.json() as QuickScanResult & { error?: string };
  ...
  return (
    <div className="mx-auto mt-10 w-full max-w-xl">
      <form onSubmit={handleScan} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Paste any URL to check accessibility…"
        ...
        >
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Scan free"}
        </Button>
      </form>
      ...
      {result && (
        <div className="mt-3 rounded-xl border border-border bg-surface-elevated p-4 shadow-soft">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground">Accessibility score</p>
              <p className={`text-3xl font-bold tracking-tight ${scoreColor}`}>{result.score}<span className="text-base font-normal text-muted-foreground">/100</span></p>
            </div>
            <div className="text-right">
              <p className="text-xs text-muted-foreground">Issues found</p>
              <p className="text-3xl font-bold text-ink">{result.totalIssues}</p>
            </div>
          </div>
          {result.preview.length > 0 && (
            <div className="mt-3 space-y-1.5 border-t border-border pt-3">
              {result.preview.map((issue) => (
                <div key={issue.id} className="flex items-center gap-2 text-xs">
                  <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${
                    ...
                    issue.severity === "medium" ? "bg-accent" : "bg-muted-foreground"
                  }`} />
                  <span className="text-ink">{issue.title}</span>
                  <span className="ml-auto text-muted-foreground">×{issue.count}</span>
                </div>
              ))}
            </div>
          )}
          {result.teaser && (
            <div className="mt-3 border-t border-border pt-3">
              <Link to="/sign-up" className="flex items-center justify-between text-xs font-medium text-accent hover:underline">
                {result.teaser} <ArrowRight className="h-3.5 w-3.5" />
              </Link>
            </div>
          )}
          {!result.teaser && (
            <div className="mt-3 border-t border-border pt-3">
              <Link to="/dashboard" className="flex items-center justify-between text-xs font-medium text-accent hover:underline">
                Sign up to save & share this report <ArrowRight className="h-3.5 w-3.5" />
              </Link>
            </div>
          )}
        </div>
...
function useDownloadExtension() {
  ...
  return () => {
    ...
    a.href = "/visionadapt-extension.zip";
    a.download = "visionadapt-extension.zip";
    a.click();
  };
}
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h2 className="mt-1 text-3xl font-semibold tracking-tight text-ink md:text-4xl">{children}</h2>;
}
function SectionLead({ children }: { children: React.ReactNode }) {
  return <p className="mt-3 max-w-2xl text-base text-muted-foreground">{children}</p>;
}
      ...
      <div className="relative mx-auto max-w-7xl px-6 pb-24 pt-20 md:pt-28">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-pretty text-lg text-muted-foreground">
            A11ai is an adaptive color engine that transforms any website in real time for
            protanopia, deuteranopia, tritanopia, and low-vision users — without breaking
            layout, branding, or function.
          </p>
      ...
      <div className="rounded-2xl border border-border bg-surface-elevated p-2 shadow-elegant">
        <div className="overflow-hidden rounded-xl border border-border bg-background">
          <div className="flex items-center justify-between border-b border-border bg-surface px-4 py-2.5">
            ...
            <div className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Adapted</div>
          </div>
...
function PreviewPane({ mode, label, highlight = false }: { mode: VisionMode; label: string; highlight?: boolean }) {
  return (
...
function LogoStrip() {
  const logos = ["CipherCore Platform", "TB AI", "NORDIC GOV", "ATLAS EDU", "QUARTZ", "LUMEN BANK"];
  return (
    <section className="border-y border-border bg-surface">
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
          {logos.map((l) => (
            <span key={l} className="text-sm font-semibold tracking-widest text-muted-foreground/70">{l}</span>
          ))}
        </div>
      </div>
    </section>
            ...
            {VISION_MODES.map((m) => (
              <button
                ...
                className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${
                  mode === m.id ? "bg-ink text-background" : "text-muted-foreground hover:bg-muted hover:text-ink"
                }`}
              >
                {m.label}
              </button>
    ...
    <div className="mx-auto max-w-3xl rounded-xl border border-border bg-surface-elevated p-6 shadow-soft">
      <div className="flex items-center justify-between border-b border-border pb-4">
        ...
        <div className="flex gap-2">
          <span className="rounded bg-destructive px-2 py-0.5 text-xs font-medium text-destructive-foreground">Critical</span>
          <span className="rounded bg-warning px-2 py-0.5 text-xs font-medium text-warning-foreground">Review</span>
          <span className="rounded bg-success px-2 py-0.5 text-xs font-medium text-success-foreground">Resolved</span>
        </div>
      </div>
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2 text-sm">
          <button className="w-full rounded-md bg-accent px-4 py-2 font-medium text-accent-foreground">Schedule consultation</button>
          <button className="w-full rounded-md border border-border bg-background px-4 py-2 font-medium text-ink">Download report</button>
          <button className="w-full rounded-md bg-destructive px-4 py-2 font-medium text-destructive-foreground">Cancel admission</button>
        </div>
      </div>
    </div>
  );
...
const FEATURES = [
  { icon: Layers, title: "DOM-aware transformation", body: "Scans stylesheets and live DOM, maps inaccessible color pairs, and injects override rules without breaking layout." },
  { icon: Palette, title: "Semantic color preservation", body: "Errors stay urgent. Success stays positive. Color shifts are paired with icons, borders, and patterns." },
  { icon: Workflow, title: "MutationObserver pipeline", body: "Reacts to SPA route changes and async content. Works with React, Vue, Svelte, and modern frameworks." },
  { icon: Gauge, title: "Sub-frame performance", body: "Transformations cached per-domain. Median injection cost under 4ms on mid-tier hardware." },
  { icon: ShieldCheck, title: "WCAG 2.2 scoring", body: "Continuous compliance scoring with downloadable audit reports for legal and procurement." },
  { icon: Zap, title: "One-line SDK", body: "Drop in a script tag or install the npm package. Profiles propagate automatically to authenticated users." },
];
...
const TIERS = [
  {
    name: "Starter", price: "Free", note: "For individuals and small projects",
    features: ["Browser extension", "5 vision profiles", "Community support"],
    cta: "Install extension",
  },
  {
    name: "Team", price: "$49", suffix: "/mo", note: "For product and design teams",
    features: ["Live Studio", "Palette generator", "WCAG reports", "Up to 10 seats"],
    cta: "Start 14-day trial", featured: true,
  },
  {
    name: "Enterprise", price: "Custom", note: "Compliance-grade deployment",
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-24">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-3">
        {TIERS.map((t) => (
          <div key={t.name} className={`rounded-2xl border p-7 ${t.featured ? "border-ink bg-ink text-background shadow-elegant" : "border-border bg-surface-elevated"}`}>
            <div className="flex items-baseline justify-between">
              <h3 className={`text-sm font-semibold ${t.featured ? "" : "text-ink"}`}>{t.name}</h3>
              {t.featured && (
                <span className="rounded-full bg-background/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider">Popular</span>
              )}
            </div>
            ...
            <Button
              className={`mt-7 w-full ${t.featured ? "bg-background text-ink hover:bg-background/90" : "bg-ink text-background hover:bg-ink/90"}`}
...
const FAQS = [
  { q: "Does A11ai break my site's layout?", a: "No. The engine performs scoped CSS overrides and runs a layout-integrity check after each injection. Branding tokens are preserved within accessibility bounds." },
  { q: "How is this different from a hue-rotate filter?", a: "Hue filters distort imagery and lose semantic meaning. Our engine analyzes contrast pairs, semantic states, and DOM context to remap only the components that need it." },
  { q: "Which frameworks are supported?", a: "Any framework producing HTML. The SDK ships with first-class adapters for React, Vue, Svelte, and Next.js." },
  { q: "Are reports admissible for compliance audits?", a: "Yes. Reports include WCAG 2.2 criterion mapping, evidence snapshots, and signed timestamps suitable for legal review." },
];
        ...
        <div className="mt-10 divide-y divide-border rounded-2xl border border-border bg-surface-elevated">
          {FAQS.map((faq, i) => (
            <div key={i} className="px-6 py-5">
              ...
              >
                {faq.q}
                <span className="ml-4 shrink-0 text-lg text-muted-foreground">{open === i ? "−" : "+"}</span>
              </button>
              {open === i && <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{faq.a}</p>}
            </div>
          ))}
        </div>
      </div>
...
function CTA() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-24">
      <div className="rounded-2xl bg-ink px-8 py-16 text-center text-background shadow-elegant md:px-16">
        <div className="mx-auto max-w-2xl">
          <h2 className="text-3xl font-semibold tracking-tight md:text-4xl">
            Make your product accessible to every user.
          </h2>
          <p className="mt-4 text-base text-background/70">
            Install the extension in seconds. One click adapts any website for 300 million people
            living with color vision deficiency.
          </p>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Button size="lg" className="h-11 bg-background px-6 text-ink hover:bg-background/90" onClick={download}>
              {user ? (
                <>Download Extension <Download className="ml-1.5 h-4 w-4" /></>
              ) : (
            ...
            <Button size="lg" variant="ghost" className="h-11 px-6 text-background hover:bg-background/10 hover:text-background" asChild>
              <Link to="/studio">Try Live Studio</Link>
            </Button>
          </div>
        </div>
      </div>
